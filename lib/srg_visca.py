...
class Camera:
    ...
    def _send_command(self, command_hex: str, query=False) -> Optional[bytes]:
        ...
        for retry_num in range(self.num_retries):
            ...
            try:
                ...
                logger.debug(f"Response: {response} -- Retry num: {retry_num}")
                if response == old_response:
                    return response
                old_response = response
                try:
                    logger.debug(f"Response byte: {response.decode('utf-8')}") if response is not None else None
                except:
                    pass
        ...
        if exception:
            ...
            logger.debug("No response after retries")
    ...
    def pantilt(self, pan_speed: int, tilt_speed: int, pan_position=None, tilt_position=None, relative=False):
        ...
        if None not in position_params:
            ...
            if not isinstance(pan_position, int):
                pan_position = 0
            if not isinstance(tilt_position, int):
                tilt_position = 0
            command = '06' + relative_hex + pan_speed_hex + tilt_speed_hex + encode(pan_position) + encode(tilt_position)
    ...
    def get_pantilt_position(self) -> Tuple[int, int]:
        ...
        print("Getting pan-tilt position")
        response = self._send_command('06 12', query=True)
        
        ## If no response we raise error
        if response is None:
            error_text = 'ERROR: Could not get pan-tilt position response'
    ...
    def get_zoom_position(self) -> int:
        ...
        ## If no response we raise error
        if response is None:
            raise NoQueryResponse('Could not get pan-tilt position response')
...
class VISCA_DEVICES:
    ...
    def _send_command(self, command_hex: str, multiline=False):
        if multiline == True:
    ...
    def _receive_response(self):
        ...
        while True:
            try:
                ...
                ## Make sure we do not read our own command
                if response == b'\x02ENQ:network\xff\x03':
                    time.sleep(1)
                    response_bin = self._sock.recv(1024)
                    response_bin = response_bin[1:-2]  # remove preamble and terminator
                    
                    ## Wash the byte like object
                    response_bin = response_bin.replace(b'\xff', b'\n') # Replace ff with newline
                    response_string = response_bin.decode('utf-8')
                    
                    ## create dictionary from response                    
                    for line in filter(None, response_string.strip().split('\n')):
    ...
    def get_visca_devices(self) -> str:
        ...
        response = self._send_command(command_hex=command_data, multiline=False)
    ...
    def set_visca_device_ip(self, device_mac: str, device_ip: str, device_mask: str, device_gateway: str, device_name: str) -> None:
        ...
        response = self._send_command(command_hex=command_data, multiline=True)
